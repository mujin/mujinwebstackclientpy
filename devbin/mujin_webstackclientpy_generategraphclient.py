...
def _ParseArguments():
    ...
    parser.add_argument('--subscription', action='store_true', help='Generate subscription GraphQL APIs')
...
def _PrintSubscrptionMethod(operationName, parameters, description, returnType):
    builtinParameterNames = ('fields', 'subscriber')
    print('    def %s(self, subscriber, %s):' % (operationName, ', '.join([
        '%s=None' % parameter['parameterName'] if parameter['parameterNullable'] else parameter['parameterName']
        for parameter in parameters
        if parameter['parameterName'] not in builtinParameterNames
    ] + ['fields=None'])))
    if description:
        ...
        print('')
        print('        Args:')
        print('            subscriber (GraphSubscriber): Subscriber that listens to subscription status and messages.')
        for parameter in parameters:
            if parameter['parameterName'] in builtinParameterNames:
                continue
            isOptionalString = ", optional" if parameter['parameterNullable'] else ""
        ...
        print('')
        print('        Returns:')
        print('            id (str): The id of the subscription, used for unsubscribe.')
        print('')
    ...
    for parameter in parameters:
        if parameter['parameterName'] in builtinParameterNames:
            continue
        print('            (%r, %r, %s),' % (parameter['parameterName'], parameter['parameterType'], parameter['parameterName']))
    print('        )')
    print('        return self._SubscribeSimpleGraphAPI(subscriber, subscriptionName=%r, parameterNameTypeValues=parameterNameTypeValues, returnType=%r, fields=fields)' % (operationName, returnType['baseTypeName']))
...
def _PrintSubscriptionClient(serverVersion, subscriptionMethods):
    print('# -*- coding: utf-8 -*-')
    print('#')
    print('# DO NOT EDIT, THIS FILE WAS AUTO-GENERATED')
    print('# GENERATED BY: %s' % os.path.basename(__file__))
    ...
    print('#')
    print('')
    print('from .subscriptionclientraw import GraphSubscriptionBase')
    print('')
    print('class GraphSubscriptionClient(GraphSubscriptionBase):')
    print('')
    for subscriptionMethod in subscriptionMethods:
        _PrintSubscrptionMethod(**subscriptionMethod)
        print('')
    print('')
    print('#')
    print('# DO NOT EDIT, THIS FILE WAS AUTO-GENERATED, SEE HEADER')
...
def _Main():
    ...
    if options.subscription:
        ...
        _PrintSubscriptionClient(serverVersion, subscriptionMethods)
    else:
